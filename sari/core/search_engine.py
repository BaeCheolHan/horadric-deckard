...
class SearchEngine:
    ...
    def search_v2(self, opts: SearchOptions) -> Tuple[List[SearchHit], Dict[str, Any]]:
        ...
        # 2. FTS Search
        fts_hits = []
        # Allow unicode in FTS, but fallback if non-ASCII character present
        ...
        if not fts_success:
            ...
            # Force fallback if FTS results are suspiciously empty for non-trivial query
            prefer_path_only = (has_unicode or is_too_short) and opts.total_mode != "exact"
            res, like_meta = self.db._search_like(opts, terms, meta, no_slice=True, prefer_path_only=prefer_path_only)
            fts_hits = res
            meta.update(like_meta)
    ...
    def _process_rows(self, rows: list, opts: SearchOptions,
        ...
        # Local import of _decompress is no longer strictly needed if content comes from VIEW,
        ...
        for r in rows:
            ...
            # Debugging fallback logic - if no matches found via count_matches, log why
