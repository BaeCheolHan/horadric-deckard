...
def test_resolve_workspace_roots_priority(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SARI_ROOT_1", str(root_env))
...
def test_resolve_workspace_roots_root_uri_keep_nested(tmp_path, monkeypatch):
    ...
    root_a.mkdir()
    root_b.mkdir()
    monkeypatch.setenv("SARI_KEEP_NESTED_ROOTS", "1")
    roots = WorkspaceManager.resolve_workspace_roots(
...
def test_resolve_workspace_roots_env(tmp_path, monkeypatch):
    root_a = tmp_path / "a"
    root_a.mkdir()
    monkeypatch.setenv("SARI_WORKSPACE_ROOT", str(root_a))
