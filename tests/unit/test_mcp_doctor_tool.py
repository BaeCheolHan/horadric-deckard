...
def test_doctor_basic_structure(tmp_path, monkeypatch):
    ...
    res = doctor.execute_doctor({"include_network": True, "include_port": True, "include_marker": True})
    payload = json.loads(res["content"][0]["text"])
    assert payload["workspace_root"] == str(tmp_path)
    assert isinstance(payload["results"], list)
    assert any(r["name"] == "DB Existence" for r in payload["results"])
    assert any(r["name"] == "Workspace Marker (.codex-root)" for r in payload["results"])
